...
|------|-----------------|---------------------|----------|
| [01-system-overview](./01-system-overview/) | 项目定位、用户场景、功能需求、插件化设计 | 功能完成度、数据现状、迭代路线图 | 🚧 v0.6 开发中 |
| [02-architecture](./02-architecture/) | 分层架构、组件职责、通信协议 | 架构实现状态、性能基线 | ✅ 稳定 |
...
| [05-frontend](./05-frontend/) | 组件规格、代码规范、路由设计、**知识图谱可视化方案** | 组件状态、**AI 配置面板待实现** | 🚧 **AI 面板最高优先级** |
| [06-ai-mcp](./06-ai-mcp/) | **三种问答模式**、MCP 协议、**用户可控配置** | **配置面板未开始**、工具清单 | 🚧 **v0.6 核心任务** |
| [07-deployment](./07-deployment/) | Docker 配置、备份策略 | 部署状态、排查清单 | ✅ 稳定 |
...
阅读顺序：
1. [01-system-overview/spec.md](./01-system-overview/spec.md) —— 先搞清楚这是什么项目
2. [01-system-overview/progress.md](./01-system-overview/progress.md) —— 摸清债务和欠账
3. [03-data-model/spec.md](./03-data-model/spec.md) —— 理解数据模型（核心+扩展）
4. [06-ai-mcp/spec.md](./06-ai-mcp/spec.md) —— 理解 AI 设计（三种模式、用户可控）

### 我要开发 v0.6（当前迭代）

**核心任务**：AI 配置面板

1. **前端**：参考 [05-frontend/progress.md](./05-frontend/progress.md) 的"AI 配置面板"章节
2. **后端**：参考 [06-ai-mcp/spec.md](./06-ai-mcp/spec.md) 的"用户配置存储"和"前端界面设计"
3. **API**：参考 [04-api-contracts/spec.md](./04-api-contracts/spec.md) 添加配置相关接口
