...
wsl -e bash -lic "docker start neo4j-add 2>/dev/null || docker run -d --name neo4j-add -p 7474:7474 -p 7687:7687 -e NEO4J_AUTH=neo4j/password neo4j:5"
if errorlevel 1 (
...
start /min "Backend" cmd /c "cd /d %~dp0backend && .venv\Scripts\activate && python -m uvicorn app.main:app --reload --port 8000"
